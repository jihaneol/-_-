...
class ShopMemberController(
    ...
    fun signup(@RequestBody request: MemberRequest): ResponseEntity<ApiResponse<Any>> =
        ResponseEntity
            .status(HttpStatus.CREATED)
            .body(ApiResponse.success(memberUseCase.createMember(CreateMemberInput(request.name, request.email)).toResponse()))
}
