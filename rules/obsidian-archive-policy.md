...
프로젝트 내부 lane별 `workflow/*/phases/` 폴더는 실행 가능한 phase 최종본만 유지한다.
...
| `workflow/backend/phases/*.md` | backend active phase 구현 계약 | 완료 후 Obsidian에 기록하고 phase 파일은 `workflow/backend/archive/YYYY-MM-DD/`로 이동 |
| `workflow/frontend/phases/*.md` | frontend active phase 구현 계약 | 완료 후 Obsidian에 기록하고 phase 파일은 `workflow/frontend/archive/YYYY-MM-DD/`로 이동 |
...
- `workflow/backend/state/run-state.md` 또는 `workflow/frontend/state/run-state.md`
...
6. 프로젝트 lane별 `state/run-state.md`와 `state/execute-state.json`을 다음 작업 기준으로 갱신한다.
...
- `workflow/backend/phases/*.md`
- `workflow/frontend/phases/*.md`
